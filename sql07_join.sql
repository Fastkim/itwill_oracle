...
-- 사번, 이름, 부서번호, 부서이름 - right outer ioin
...
-- 1) ANSI:
select e.empno, d.dname, e.ename, sal
from emp e
...
-- right join:
select e1.empno, e1.ename as "직원 이름", e2.mgr, e2.ename as "매니저 이름"
from emp e1
...
-- 1) ANSI:
select d.deptno, d.dname, count(*), min(e.sal) as "MIN", max(e.sal) as "MAX" -- count, min, max 모두 group by 결과값을 산출
from dept d
