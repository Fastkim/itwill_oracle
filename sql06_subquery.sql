...
-- 연봉 내림차순으로 출력.
select sal * 12 + nvl(comm, 0) as "YEAR_COMM"
from emp
where ename = 'WARD';

select empno, ename, sal, comm, sal * 12 + nvl(comm, 0) as "YEAR_COMM"
from emp
...
select ename,to_char(hiredate,'YYYY/MM/DD') as "INCOMPANYDAY"
from emp
...
-- 다중행 서브퀘리: 서브쿼리의 결과가 2개 이상의 행인 경우.
